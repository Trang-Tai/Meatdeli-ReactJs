...
import './InstantFood.scss';

const searchProductLine = [
    PRODUCT_LINE.INSTANT_FOOD,
]

// sort(() => 0.5 - Math.random()).

function InstantFood(props) {
    ...
    return (
        <div className="instant-food-container">
            <div className="grid wide">
                <div className="instant-food-content">
                    <span className="instant-food-title" onClick={() => navigate(`/collections/${searchProductLine.join('-')}`)}>Sản Phẩm Ăn Liền</span>
                    <div className="product-card-list row">
                        <div className="col c-4-8 m-4-8 s-12">
                            <img onClick={() => navigate(`/collections/${searchProductLine.join('-')}`)} className="instant-food-banner-img" src="https://theme.hstatic.net/200000507659/1000893368/14/section_hot.jpg?v=219" />
                        </div>
                        {
                            customListProducts && customListProducts.length > 0 &&
                            [...customListProducts].slice(0, 8).map((item, index) => {
                                return (
                    ...
                    <div className="instant-food-view-all">
